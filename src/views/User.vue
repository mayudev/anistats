<template>
  ...
  <div class="view" v-else>
    <div
      ...
      :class="{ 'sidebar-hide-mobile': !showSidebar }"
    >
      <Sidebar
        @special="toggleSpecial($event)"
  ...
  methods: {
    ...
    loadActivites(): void {
      fetchActivity(
        ...
        this.preloadActivites(activities).then((preloaded) => {
    ...
    async preloadActivites(activities: ActivityDay[]): Promise<ActivityDay[]> {
      ...
      const nextPage = await fetchActivity(
        this.state.userData.id,
        this.state.currentPage + 1,
        this.state.mediaType
      );
      const preloadedActivities = store.parseActivities(
        nextPage.data.Page.activities
      );
      ...
      const duplicates = preloadedActivities.filter((x) =>
        compareDates(x.day, activities[activities.length - 1].day)
      );
